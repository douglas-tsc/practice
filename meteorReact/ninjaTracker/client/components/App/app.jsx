App = React.createClass({
    ...
    if (this.data.requests.length) {
      return this.data.requests.map((request) => {
        ...
        if (request.assignedNinja == this.data.currentNinja._id) {
          return (
            <div key={request._id}>
              <h1 className="request" id={request._id}>{request.target}</h1>
              <button onClick={this.completeRequest} className="btn btn-primary">Target Eliminated</button>
            </div>
          )
          foundRequest = true;
        } else {
          if (count == numRequests && !foundRequest)  {
            return (
              <h1 key={1}>You do not have any assigned targets.</h1>
            )
          }
        }
        });
  ...
  completeRequest(e) {
    ...
    request.ninja = ninja;
    Meteor.call('completeRequest', request);
  },
  showLayout() {
