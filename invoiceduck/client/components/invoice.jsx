Invoice = React.createClass({
    ...
    getMeteorData() {
        ...
        return {invoice: Invoices.findOne(), items: InvoiceItems.find().fetch()};
    },
    getItems() {
        ...
        if (items.length > 0) {
            return items.map((item, index) => {
                return <InvoiceItem ref='InvoiceItemRow'
                    key={item._id}
                    id={item._id}
                    description={item.description}
                    qty={item.qty} price={item.price} handleRemove={this.handleRemoveItem}
                    />;
            });
        }
    },
    ...
    render() {
        ...
        return <div className='Invoice'>
            ...
            <input type='text' name='invoiceSubject' className='form-control' placeholder='Invoice description'/>
            <br/>
